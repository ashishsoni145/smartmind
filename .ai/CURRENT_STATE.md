...
- Android production distribution and client/server secret boundary (2026-09-25, ADR 0011, branch `arena/01a0d9f2-smartmind`):
  ...
  - Not verified and not claimed: no Gradle build ran (the sandbox has no JDK/Android SDK and cannot reach dl.google.com, services.gradle.org, maven.google.com or Maven Central), so `testDebugUnitTest`, `assembleDebug`, `assembleQaStandalone` and `bundleRelease` are unproven here and GitHub Actions is the proof path; no real APK/AAB was scanned (the auditor is proven against fixtures it builds itself); no physical-device test was performed; and `android-qa`/`android-release` will fail at the configuration gate until the public Supabase anon key is supplied — which is the intended behaviour. The sandbox bot token cannot list or set repository secrets (HTTP 403).
  - Shipped as commit `b198f71` on PR #15 (https://github.com/ashishsoni145/smartmind/pull/15). CI run 36184628852 on that PR: `verify` **success** (mobile/web/backend typecheck, Jest, the 28 Node build-script tests, `audit:selftest`, `audit:source --format github`, backend 252 tests); `android-qa` **failure at the configuration gate**, which is the designed behaviour with the anon key still empty — every later step was skipped; `android-debug` `Kotlin unit tests` **success**, proving Gradle 9.4.1 + AGP 9.2.1 + the RNGP `settings.gradle` resolution work and that the whole `android { }` block including the new `qaStandalone` build type configures cleanly. `Assemble debug APK` was still running when the sandbox GitHub token expired (HTTP 401), so its result was not read.

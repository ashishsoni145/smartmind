...
interface VisualLearningViewerProps {
  title?: string;
  topicTitle?: string;
  onClose?: () => void;
}
...
export const VisualLearningViewer: React.FC<VisualLearningViewerProps> = ({
  ...
  topicTitle = 'Motion in a Plane (NCERT Chapter 4)',
  onClose,
}) => {
  ...
  // 3D Camera Orbit Controls (User can rotate camera angle!)
  const [cameraYaw, setCameraYaw] = useState<number>(40); // degrees
  const [cameraPitch, setCameraPitch] = useState<number>(24); // degrees
  const [cameraZoom, setCameraZoom] = useState<number>(380); // camera distance
  ...
  const calculatePositionAt = useCallback(
    (t: number) => {
      ...
      if (!airDrag) {
        ...
        // Linear drag approximation k = 0.08
        const k = 0.08;
  ...
  // Mouse / Touch drag to orbit camera angle in 3D void
  ...
  useEffect(() => {
    ...
    if (viewMode === '3d_void') {
      // ==========================================
      // 3D VOID RENDER MODE (ORBITABLE CAMERA)
      // ==========================================
      const originX = width / 2;
      const originY = height * 0.65;
      const scaleM2U = Math.min(width, height) / 110; // Scaling factor

      // 1. Draw 3D Ground Grid (Y = 0)
      ctx.lineWidth = 1;
      const gridSize = 120; // meters
      const gridStep = 15;
      ...
      // Velocity vectors (vx, vy, v_resultant)
      const vScale = 1.2;
      ctx.strokeStyle = '#10b981'; // vx
      ctx.lineWidth = 2;
      ctx.beginPath();
      ...
      ctx.strokeStyle = '#f59e0b'; // v resultant
      ctx.beginPath();
      ctx.moveTo(curSX, curSY);
  ...
  return (
    <div className={styles.container} role="region" aria-label="Interactive Visual Trajectory Simulator">
      {/* Top Header */}
      <div className={styles.topBar}>
        <div className={styles.titleArea}>
          <span className={styles.badge}>Interactive 3D Simulation</span>
          <div>
      ...
      <div className={styles.mainStage}>
        ...
        {/* 3D Camera Controls & Rotation Instructions */}
        {viewMode === '3d_void' && (
          <>
      ...
      <div className={styles.controlsPanel}>
        <div className={styles.slidersGrid}>
          ...
          {/* Azimuth Angle φ (for 3D deflection) */}
          {viewMode === '3d_void' && (
            <div className={styles.controlGroup}>
              <div className={styles.controlHeader}>
        ...
        <div className={styles.playControlsRow}>
          ...
          <div className={styles.theoryPill}>
            <Icon name="info" size="xs" />
            <span>
              Parabolic Path: y = x·tan(θ) - [g / (2·v₀²·cos²θ)]·x²
            </span>
          </div>
        </div>
      </div>
